...
public class FieldService : IFieldService
{
    ...
    public IEnumerable<FieldDto> GetAll() =>
        _fields.Values.Select(f => new FieldDto(
            f.Id,
            f.Name,
            GetSize(f.Id),
            new LocationDto(
                new CoordinateDto(f.Center.Y, f.Center.X),
                f.Polygon.Coordinates.Select(c => new CoordinateDto(c.Y, c.X))
            )
        ));
    ...
    public ContainsDto? Contains(double lat, double lng)
    {
        ...
        {
            return new ContainsDto(f.Id, f.Name);
        }
        return null;
    }
